"""TrustScoreAgent tools for CrewAI.
...
Usage::
    ...
    agent = Agent(
        role="Service scout",
        goal="Only call trustworthy APIs",
        tools=get_trustscoreagent_tools(),
    )

Requires ``crewai`` and ``requests`` (see requirements.txt).
"""
...
class TrustScoreCheckReputationTool(BaseTool):
    name: str = "trustscore_check_reputation"
    description: str = (
        "Check the trust score of an AI microservice or public API BEFORE calling "
        "it. Returns score (0-1), confidence, rating count and an "
        "availability/latency/conformity breakdown."
    )
    args_schema: Type[BaseModel] = CheckReputationInput
    _api: _TrustScoreAPI = PrivateAttr()

    def __init__(self, api: _TrustScoreAPI, **kwargs) -> None:
        super().__init__(**kwargs)
        self._api = api

    def _run(self, service: str) -> str:
        return self._api.check_reputation(service)
...
class TrustScoreSubmitRatingTool(BaseTool):
    ...
    def _run(
        ...
        return self._api.submit_rating(
            service,
            status_code,
            latency_ms,
            response_size_bytes,
...
class TrustScoreListServicesTool(BaseTool):
    name: str = "trustscore_list_services"
    description: str = (
        "List rated services sorted by trust score, to discover reliable services "
    ...
    def _run(
        ...
        return self._api.list_services(sort_by, min_score, min_ratings, limit)
...
def get_trustscoreagent_tools(
    ...
    """Return the three TrustScoreAgent tools as CrewAI ``BaseTool``s."""
    api = _TrustScoreAPI(base_url=base_url, agent_did=agent_did, timeout=timeout)
    return [
        TrustScoreCheckReputationTool(api),
        TrustScoreSubmitRatingTool(api),
        TrustScoreListServicesTool(api),
    ]


__all__ = [
    "get_trustscoreagent_tools",
    "TrustScoreCheckReputationTool",
    "TrustScoreSubmitRatingTool",
    "TrustScoreListServicesTool",
]
